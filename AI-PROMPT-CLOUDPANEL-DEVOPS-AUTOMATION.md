# Electron App: CloudPanel DevOps Automation Center
...
1. **CloudPanel Setup**: Manual domain registration and virtual environment creation
2. **VPS Configuration**: Manual SSH key setup, user access, git repo cloning
3. **GitHub Secrets**: Manual creation and configuration of all secrets
4. **Workflow Files**: Manual YAML configuration that often fails on first run
5. **Multiple Tools**: Switching between Termius, CloudPanel UI, GitHub for different tasks

## Project: All-in-One CloudPanel + GitHub Actions Automation Tool

### Core Problem to Solve
Create an Electron desktop application that automates the entire deployment pipeline from domain registration to successful GitHub Actions deployment, integrating directly with CloudPanel API, AWS EC2, and GitHub - eliminating ALL manual steps.

## Technology Stack
- **Framework**: Electron.js with React + TypeScript
- **Styling**: Tailwind CSS (light theme)
- **CloudPanel Integration**: CloudPanel API v2
...
### 1. CloudPanel Automation Module
```typescript
interface CloudPanelAutomation {
  // One-click domain setup
  quickSetup: {
    domainRegistration: (domain: string) => Promise<void>;
    createVirtualHost: (domain: string, phpVersion: string) => Promise<void>;
    setupSSL: (domain: string) => Promise<void>;
    createDatabase: (dbName: string) => Promise<void>;
    createFtpUser: (username: string) => Promise<void>;
  };
  
  // CloudPanel API integration
  api: {
    listSites: () => Promise<Site[]>;
    listDatabases: () => Promise<Database[]>;
    restartServices: (service: 'nginx' | 'mysql' | 'php-fpm') => Promise<void>;
    viewLogs: (type: 'error' | 'access') => Promise<string>;
  };
}
```

### 2. Intelligent SSH & Git Setup
```typescript
interface AutoSSHSetup {
  // Completely automated SSH setup
  autoConfig: {
    generateKeys: () => Promise<KeyPair>;
    uploadToEC2: (instanceId: string) => Promise<void>;
  ...
  // Smart repo cloning
  smartClone: {
    detectOptimalPath: (cloudPanelSite: string) => string; // Auto-detect /home/cloudpanel/htdocs/...
...
### 3. GitHub Secrets Auto-Configuration
```typescript
interface SecretsAutomation {
  // Auto-detect and create all needed secrets
  autoDetect: {
    scanEC2: () => Promise<RequiredSecrets>; // HOST, USERNAME, etc.
    scanCloudPanel: () => Promise<CloudPanelSecrets>; // DB credentials, paths
...
### 4. Intelligent Workflow Generator
```typescript
interface WorkflowAI {
  ...
  generateWorkflow: {
    ...
    customizeForCloudPanel: (workflow: string, siteConfig: Site) => string;
    addCaching: () => string;
    addTestStage: () => string;
  ...
  autoFix: {
    ...
    phpVersionConflicts: () => Promise<void>;
  };
}
```

### 5. One-Click Full Stack Deployment
```typescript
interface MagicDeployment {
  ...
  deployNewProject: async (config: {
    ...
    // 5. Clone repository
    // 6. Setup proper permissions
    // 7. Create all GitHub secrets
    // 8. Generate optimal workflow
    // 9. Commit workflow file
    // 10. Trigger first deployment
    // 11. Monitor and auto-fix any issues
...
### Smart Terminal View with CloudPanel Integration
```typescript
interface SmartTerminal {
  // Embedded Termius-like experience
  features: {
    multiTab: boolean; // Multiple SSH sessions
    cloudPanelShortcuts: string[]; // Quick commands for CloudPanel
  ...
  quickCommands: {
    restartNginx: 'clpctl system:service:restart nginx';
    clearCache: 'cd /home/cloudpanel/htdocs/{site} && php artisan cache:clear';
...
2. **AWS EC2 Integration** - List instances, get details
3. **CloudPanel API Client** - Full API wrapper
...
2. **CloudPanel Site Creation** - API automation
...
## Specific CloudPanel Workflows to Automate

### Laravel Project Deployment
```yaml
automation:
  - Create MySQL database via CloudPanel API
  - Set PHP version to 8.2
  - Clone repository to /home/cloudpanel/htdocs/{domain}
...
automation:
  ...
  - Install WordPress via CloudPanel
...
1. **Permission Errors**
   ...
   - Set correct ownership (cloudpanel:cloudpanel)

2. **Path Issues**
   - Auto-detect CloudPanel htdocs structure
   - Fix relative vs absolute paths

3. **PHP Version Mismatches**
   - Detect required version from composer.json
   - Auto-switch PHP version in CloudPanel
...
- **CloudPanel API coverage** - All essential endpoints
...
1. **CloudPanel API integration** - Most critical
